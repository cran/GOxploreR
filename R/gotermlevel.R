...
GOTermBPOnLevel <- function(goterm){
  ...
  if(length(isna) > 0 && length(nonretired) > 0){
    ...
    stop(paste(c("Check that the term on index",index,"are bp GO-terms and not obsolete"), collapse = " "))
  }
  else if(length(isna) > 0 ){
    stop(paste(c("Check that the term on index", isna,"are bp GO-terms and not obsolete"), collapse = " "))
  }else if(length(nonretired) > 0){
    stop(paste(c("Check that the term on index", nonretired,"are bp GO-terms and not obsolete"), collapse = " "))
  }
